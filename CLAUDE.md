...
Exits with code 1 if any test fails. Tests are split into unit tests (against a temp vault) and integration tests (against `$HOME/notes`).

## Architecture

Zsh shell functions for searching and navigating a personal notes vault (`$HOME/notes`), where notes are Markdown files with YAML frontmatter.

**`functions/notes-lib.zsh`** — core library:
